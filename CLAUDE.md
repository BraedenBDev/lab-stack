...
**Middleware order is load-bearing.** The chain in `index.ts` is `secureHeaders` (all responses) → `hono/logger` → `cors` (`/api/*`) → Better Auth catch-all → app routes. CORS **must** precede the auth handler, which is mounted as `.on(["POST","GET"], "/api/auth/*", c => auth.handler(c.req.raw))` and owns every `/api/auth/*` path (sign-up/in/out, sessions, OAuth callbacks, verify, reset). Public (no `requireAuth`): `/api/health`, `/api/docs` (Swagger UI), `/api/openapi.yaml` (the spec, read from repo root via `Bun.file` — also COPYed into the Docker image). Your own routes live under other `/api/*` prefixes. `secureHeaders` ships no CSP so the Vite bundle + CDN-loaded Swagger UI work.
